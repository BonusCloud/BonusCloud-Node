...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
        ...
        <script type="text/javascript">
			...
			function init_show() {
				var bxc_bcode = bxc_db["bxc_bcode"];
				if (typeof(bxc_bcode) != 'undefined' && bxc_bcode != '') {
					...
					if (bxc_log_lev == "debug") {
						document.getElementById("bound_log")[1].selected=true;
					} else {
						document.getElementById("bound_log")[0].selected=true;
					}
				}
			}
			...
			function bound_bcode(bcode) {
				...
				var wan_mac = bxc_db["bxc_wan_mac"];
			...
			function bxc_show_status() {
				...
				*/
				...
				var	bxc_onboot = bxc_db["bxc_onboot"];
				if (bxc_onboot == "yes") {
				...
				status_check_interval = document.getElementById("status_interval").value * 1000;
			...
			function bxc_option(action) {
				...
				var data = {"id": id, "method": "bxc.sh", "params":[action], "fields":{"bxc_option": action}};
				$.ajax({
					url: "/_api/",
					cache: false,
					type: "POST",
			...
			function bxc_bound_log() {
				if(document.getElementById("bound_log")[0].selected){
					bxc_option("errorlog");
				} else {
					bxc_option("debuglog");
				}
			...
			function dbus_write(key, value) {
				...
				var id = parseInt(Math.random() * 100000000);
				var postData = {"id": id, "method": "bxc.sh", "params":[], "fields": data};
				$.ajax({
					url: "/_api/bxc_",
					type: "POST",
					cache: false,
			...
			function dbus_read(key=""){
				...
				$.ajax({
					...
					success: function(data) {
						bxc_db = data.result[0];
						if (key == "") {
							res = bxc_db;
						} else if(typeof(bxc_db[key]) !== "undefined") {
							res = bxc_db[key];
						} else {
							res = "";
						}
			...
			function online_query(){
				...
				if (typeof(bcode) !== 'undefined' && bcode !== "") {
					document.getElementById("online_msg").style.display="none";
					document.getElementById("online_table").style.display="";
					$.ajax({
						...
						success: function(data) {
							...
							$("#online_tbody").empty();
							tableRef = document.getElementById("online_tbody");
					...
					document.getElementById("online_table").style.display="none";
					document.getElementById("online_msg").style.display="";
				}
			}

			function get_log(refresh) {
				$.ajax({
					url: '/_temp/bxc_log.txt',
					type: 'GET',
					async: true,
					cache: false,
					dataType: 'text',
